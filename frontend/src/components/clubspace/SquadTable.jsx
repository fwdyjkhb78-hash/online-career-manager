...
import { Trash2, ChevronUp, ChevronDown, Zap, RotateCcw, Sparkles, Wrench, Loader2, HandCoins, ArrowRightLeft } from 'lucide-react';
import { base44 } from '@/api/base44Client';
import { useMutation, useQueryClient } from '@tanstack/react-query';
import { toast } from 'sonner';
import { motion } from 'framer-motion';
import PlayerStatsEditor from './PlayerStatsEditor';
import LoanListToggle from './LoanListToggle';
...
export default function SquadTable({ players, clubId, canEdit, canDelete, canEditRole, onManage, onDelete, playerMorales = {} }) {
              ...
              <table className="w-full min-w-[500px]">
                <thead>
                  <tr className="border-b border-slate-700/30">
                    ...
                    {(canEdit || canEditRole) && <th className="pr-3 py-2 w-16" />}
                  </tr>
                </thead>
                <tbody>
                  {sorted.map((player, idx) => (
                    <React.Fragment key={player.id}>
                      <tr
                      onClick={() => {
                        if (hasDetailedStats(player)) {
                          setExpandedStatsId(expandedStatsId === player.id ? null : player.id);
                        }
                      }}
                      ...
                      >
                            ...
                            <div className="min-w-0">
                              <div className="flex items-center gap-1">
                                ...
                                {hasDetailedStats(player) && (
                                  <span title="Stats détaillées disponibles" className="text-slate-500 group-hover:text-slate-400 shrink-0">
                                    {expandedStatsId === player.id
                        ...
                        <td className="px-2 py-3 text-center" onClick={e => e.stopPropagation()}>
                         <div className="flex flex-col items-center gap-1">
                           ...
                           {player.is_on_loan_list && (
                             <Badge className="bg-blue-500/20 text-blue-400 border border-blue-500/40 text-xs whitespace-nowrap">
                               🤝 Prêt dispo
                             </Badge>
                           )}
                           {(() => {
                             const roleLabels = {
                               espoir: { label: 'Espoir', color: 'text-green-400' },
                               reserviste: { label: 'Réserviste', color: 'text-blue-400' },
                               rotation: { label: 'Rotation', color: 'text-yellow-400' },
                               important: { label: 'Important', color: 'text-orange-400' },
                               titulaire_indiscutable: { label: 'Titulaire', color: 'text-purple-400 font-bold' },
                             };
                             const role = roleLabels[player.player_role];
                             if (!role) return null;
                             return <span className={`text-xs ${role.color}`}>{role.label}</span>;
                           })()}
                         </div>
                        </td>
                        ...
                        {(canEdit || canEditRole) && (
                          <td className="pr-3 py-3" onClick={e => e.stopPropagation()}>
                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              {(canEdit || canEditRole) && onManage && (
                                <button
                                  title="Gérer (transfert / prêt)"
                                  onClick={() => onManage(player)}
                                  className="p-1.5 bg-slate-700 hover:bg-cyan-500/30 text-slate-400 hover:text-cyan-300 rounded transition-colors"
                                >
                                  <ArrowRightLeft className="w-3.5 h-3.5" />
                                </button>
                              )}
                              {canEdit && <PlayerStatsEditor player={player} clubId={clubId} />}
                              {canDelete && (
                                <button
                      ...
                      {/* Stats détaillées - tous les joueurs */}
                      {hasDetailedStats(player) && expandedStatsId === player.id && (
                        <tr key={`stats-all-${player.id}`}>
                          <td colSpan={canEdit ? 10 : 9} className="p-0">
                            <motion.div
                              initial={{ opacity: 0, height: 0 }}
                              animate={{ opacity: 1, height: 'auto' }}
                              exit={{ opacity: 0, height: 0 }}
                              className="border-t border-slate-600/30 bg-slate-800/60 px-5 py-4 overflow-hidden"
                            >
                              {/* Header joueur style CDF */}
                              <div className="flex flex-wrap items-center gap-4 mb-4">
                                <div className="flex items-center gap-3">
                                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-white font-black text-lg ${getOverallColor(player.overall)}`}>
                                    {player.overall}
                                  </div>
                                  <div>
                                    <p className="text-white font-bold text-base">{player.name}</p>
                                    <p className="text-slate-400 text-xs">
                                      {[player.nationality, player.age && `${player.age} ans`, player.height && `${player.height}cm`, player.weight && `${player.weight}kg`].filter(Boolean).join(' • ')}
                                    </p>
                                  </div>
                                </div>
                                {player.potential && (
                                  <div className="ml-auto text-right">
                                    <p className="text-violet-400 font-black text-xl">{player.potential}</p>
                              ...
                              {player.player_details && (
                                <p className="text-slate-400 text-xs italic mb-4 leading-relaxed border-l-2 border-slate-600 pl-3">{player.player_details}</p>
                              )}

                              {/* Grille stats 4 colonnes comme CDF */}
                              {(() => {
                                const isGK = player.position === 'GK';
                                const statsToShow = ACADEMY_STATS_FIELDS.filter(key => {
                                  const isGkStat = ['plongeon','jeu_a_la_main','degagement','reflexe_g'].includes(key);
                                  if (isGkStat && !isGK) return false;
                                  return player[key] != null && player[key] > 0;
                                });
                                // Réorganiser en 4 colonnes
                                const cols = [[], [], [], []];
                                statsToShow.forEach((key, i) => cols[i % 4].push(key));
                                return (
                                  <div className="grid grid-cols-2 md:grid-cols-4 gap-0 border border-slate-700/50 rounded-xl overflow-hidden">
                                    {statsToShow.map((key, i) => {
                                      const val = player[key];
                                      const valColor = val >= 80 ? 'text-yellow-400' : val >= 70 ? 'text-emerald-400' : val >= 55 ? 'text-blue-400' : val >= 40 ? 'text-slate-300' : 'text-red-400';
                                      return (
                                        <div key={key} className={`flex items-center justify-between px-4 py-2.5 border-b border-r border-slate-700/30 ${i % 2 === 0 ? 'bg-slate-800/80' : 'bg-slate-800/40'}`}>
                                          <span className="text-slate-400 text-xs">{ACADEMY_STAT_LABELS[key]}</span>
                                          <span className={`font-bold text-sm ml-2 ${valColor}`}>{val}</span>
                                        </div>
                                      );
                                    })}
                                  </div>
                      ...
                      {/* Stats détaillées CDF (academy only) */}

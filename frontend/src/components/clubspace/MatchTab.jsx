  ...
  return (
    <div className="flex items-center gap-2 py-0.5">
      ...
      <input
        ...
        placeholder="—"
        className="w-14 bg-slate-600 border border-slate-500 rounded px-1 py-0.5 text-white text-xs text-center focus:outline-none focus:border-emerald-500"
      />
    </div>
  );
}

function MatchCard({ match, club, user, players = [], onSubmit, onDelete, canDelete = true, loading, deleting, defaultLineup = [] }) {
  ...
  // Vérification limite joueurs academy (4 titu max, 2 remplaçants max)
  const academyStarterCount = starters.filter(name => players.find(p => p.name === name && p.is_academy)).length;
  const academyBenchCount = bench.filter(name => players.find(p => p.name === name && p.is_academy)).length;
  const academyOk = academyStarterCount <= 4 && academyBenchCount <= 2;
  const canSubmit = score.includes('-') && starters.length === 11 && bench.length >= 1 && academyOk;
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            {match.match_type === 'tournoi' ? (
              ...
              <div className="bg-amber-500/15 text-amber-300 rounded-lg px-2 py-1 text-xs font-semibold whitespace-nowrap border border-amber-500/30">
                🏆 {match.tournament_name}
              </div>
            ) : (
        ...
        <div className="border-t border-slate-700/50 p-4 space-y-4">
          {confirmed ? (
            <div className="space-y-4">
              ...
              {/* Notes joueurs */}
              {(match.home_ratings && Object.keys(match.home_ratings).length > 0) && (
                <div className="bg-slate-700/30 rounded-xl p-3">
                  <p className="text-slate-400 text-xs mb-2">⭐ Notes — {match.home_club_name}</p>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
                    {Object.entries(match.home_ratings).sort((a,b)=>b[1]-a[1]).map(([name, note]) => (
                      <div key={name} className="flex items-center justify-between text-xs">
                        <span className="text-slate-300 truncate">{name}</span>
                        <span className={`font-bold ml-2 ${note >= 7 ? 'text-emerald-400' : note >= 5 ? 'text-yellow-400' : 'text-red-400'}`}>{note}</span>
                      </div>
                    ))}
                  </div>
                </div>
              )}
              {(match.away_ratings && Object.keys(match.away_ratings).length > 0) && (
                <div className="bg-slate-700/30 rounded-xl p-3">
                  <p className="text-slate-400 text-xs mb-2">⭐ Notes — {match.away_club_name}</p>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
                    {Object.entries(match.away_ratings).sort((a,b)=>b[1]-a[1]).map(([name, note]) => (
                      <div key={name} className="flex items-center justify-between text-xs">
                        <span className="text-slate-300 truncate">{name}</span>
                        <span className={`font-bold ml-2 ${note >= 7 ? 'text-emerald-400' : note >= 5 ? 'text-yellow-400' : 'text-red-400'}`}>{note}</span>
                      </div>
            ...
            <div className="space-y-4">
              ...
              {academyStarterCount > 4 && (
                <p className="text-red-400 text-xs">⚠️ Maximum 4 joueurs du centre de formation en titulaires ({academyStarterCount}/4)</p>
              )}
              <PlayerPicker
                players={players}
              ...
              {academyBenchCount > 2 && (
                <p className="text-red-400 text-xs">⚠️ Maximum 2 joueurs du centre de formation sur le banc ({academyBenchCount}/2)</p>
              )}
              <PlayerPicker
                players={players}
              ...
              {(starters.length > 0 || bench.length > 0) && (
                <div>
                  <p className="text-slate-300 text-sm font-medium mb-2">⭐ Notes des joueurs (optionnel, /10)</p>
                  <div className="bg-slate-700/50 border border-slate-600 rounded-xl p-3 space-y-1 max-h-52 overflow-y-auto">
                    {[...starters, ...bench].map(name => (
                      <RatingInput
                        key={name}
                        name={name}
                        value={ratings[name] ?? ''}
                        onChange={val => setRatings(prev => ({ ...prev, [name]: val }))}
                      />
                    ))}
                  </div>
                </div>
...
export default function MatchTab({ club, user, clubs = [], canDelete = true, canCreate = true }) {
  ...
  const submitResult = useMutation({
    mutationFn: async ({ match, isHome, homeScore, awayScore, scorers, stats, starters = [], bench = [], redCards = [], ratings = {} }) => {
      ...
      if (isHome) {
        ...
        updateData.home_ratings = ratings;
        ...
        updateData.away_ratings = ratings;
        updateData.status = match.status === 'home_submitted' ? 'confirmed' : 'away_submitted';
      }

      // Mettre à jour la moyenne des joueurs notés
      for (const [playerName, note] of Object.entries(ratings)) {
        if (note === '' || note === undefined) continue;
        const player = players.find(p => p.name === playerName);
        if (!player) continue;
        const oldCount = player.rating_count || 0;
        const oldAvg = player.avg_rating || 0;
        const newCount = oldCount + 1;
        const newAvg = Math.round(((oldAvg * oldCount) + parseFloat(note)) / newCount * 10) / 10;
        await base44.entities.Player.update(player.id, { avg_rating: newAvg, rating_count: newCount });
      }
      ...
      // Messages joueurs post-match (anti-doublon, uniquement pour les matchs de championnat)
      const isMyMatch = match.home_club_id === club.id || match.away_club_id === club.id;
      if (isMyMatch && match.match_type !== 'tournoi') {
        ...
        if (existingMsgs.length === 0) {
          ...
          for (const player of shuffled) {
            ...
            // Contexte selon le rôle du joueur
            const roleDescriptions = {
              espoir: "Tu es un jeune espoir qui essaie de te faire une place. Tu es ambitieux, tu acceptes les décisions mais tu veux progresser.",
              reserviste: "Tu es réserviste, tu sais que tu joues peu mais tu restes professionnel et tu attends ta chance.",
              rotation: "Tu es un joueur de rotation, tu veux jouer plus mais tu comprends les choix tactiques.",
              important: "Tu es un joueur important du club, tu as de l'influence et tu t'exprimes avec maturité et assurance.",
              titulaire_indiscutable: "Tu es titulaire indiscutable, le pilier de l'équipe. Tu as du caractère, tu t'exprimes avec autorité et franchise."
            };
            const roleCtx = roleDescriptions[player.player_role] || roleDescriptions['rotation'];

            const result = await base44.integrations.Core.InvokeLLM({
              prompt: `Tu es un joueur de football FIFA nommé ${player.name} (${player.position}, note ${player.overall}, rôle: ${player.player_role || 'rotation'}). Tu joues pour ${club.name}. ${roleCtx} ${context} Écris un message court (2-3 phrases max) en français que tu enverrais à ton manager. Adapte ton ton à ton rôle et à la situation.`,

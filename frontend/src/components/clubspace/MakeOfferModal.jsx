...
export default function MakeOfferModal({ open, onClose, myClub, user }) {
  ...
  const [loanClauses, setLoanClauses] = useState({ evolutions: '', journees: '', buts: '', pd: '', penalite: '' });
  ...
  const { data: sansLigueClubs = [] } = useQuery({
    queryKey: ['sans-ligue-clubs-offer'],
    queryFn: () => base44.entities.SansLigueClub.filter({ is_active: true }, 'name', 500),
    enabled: open,
  });

  const sansLigueClubNames = new Set(sansLigueClubs.map(c => c.name));
  ...
  const otherPlayers = allPlayers.filter(p => {
    ...
    const isHorsLigue = sansLigueClubNames.has(p.club_name);
    // Club sans entraîneur en ligue : seulement avec clause
    if (!isHorsLigue && clubsWithoutManager.has(p.club_id) && !p.release_clause) return false;
    return (
  ...
  // Vrai si le joueur vient d'un club hors-ligue
  const isHorsLiguePlayer = selectedPlayer && sansLigueClubNames.has(selectedPlayer.club_name);

  const isClauseOnlyPurchase = selectedPlayer &&
    !sansLigueClubNames.has(selectedPlayer.club_name) &&
    clubsWithoutManager.has(selectedPlayer.club_id) &&
    selectedPlayer.release_clause > 0;

  // Joueur avec clause dans un club AVEC entraîneur (ligue) ou hors-ligue
  ...
  const sendOffer = useMutation({
    mutationFn: async () => {
      ...
      const isHorsLigue = sansLigueClubNames.has(selectedPlayer.club_name);
      // Achat direct par clause uniquement si le club vendeur n'a PAS de manager (pas de négociation possible)
      // Si le club a un manager, même avec clause, on passe par une offre Transfer normale
      const isDirectClauseBuy = !isHorsLigue && !sellerClub.manager_id && selectedPlayer.release_clause > 0;
      const isReleaseModeLocal = isDirectClauseBuy || (hasClauseOption && useReleaseClause);

      // ── CAS JOUEUR SANS CLUB : enchère d'officialisation directe ──
      const isFreeAgent = !selectedPlayer.club_id;
      if (isFreeAgent) {
        const now = new Date().toISOString();
        const endsAt = new Date(Date.now() + 60 * 60 * 1000).toISOString();
        const offerAmount = parseInt(amount) || 0;
        await base44.entities.Auction.create({
          ...
          starting_price: offerAmount, current_price: offerAmount,
          current_bidder_id: buyerClub.id,
          current_bidder_name: (effectiveBuyingClub || myClub)?.manager_name || '',
          current_bidder_club: buyerClub.name,
          last_bid_at: now, ends_at: endsAt,
          is_external_player: false, transfer_type: 'ligue', status: 'active',
          is_loan: false, reactions: {}
        });
        return;
      }

      // ── CAS HORS-LIGUE : offre auto-acceptée → enchère directe ──
      if (isHorsLigue && !isDirectClauseBuy) {
        const now = new Date().toISOString();
        const endsAt = new Date(Date.now() + 60 * 60 * 1000).toISOString();
        const offerAmount = parseInt(amount) || 0;

        if (isLoanMode) {
          // Prêt hors-ligue : officialisation directe
          if (offerAmount > 0) {
            await base44.entities.Club.update(buyerClub.id, { budget: (buyerClub.budget || 0) - offerAmount });
          }
          ...
          await base44.entities.Auction.create({
            ...
            player_nationality: selectedPlayer.nationality || '',
            player_image_url: selectedPlayer.image_url || '',
            seller_club_id: selectedPlayer.club_id || 'horsligue',
            seller_club_name: selectedPlayer.club_name,
            starting_price: offerAmount, current_price: offerAmount,
            current_bidder_club: buyerClub.name,
            last_bid_at: now, ends_at: now,
            is_external_player: true, transfer_type: 'hors_ligue', status: 'completed',
            is_loan: true,
            loan_buy_option: loanBuyOption ? parseInt(loanBuyOption) : 0,
            loan_mandatory_buy_option: loanMandatoryBuyOption ? parseInt(loanMandatoryBuyOption) : 0,
            reactions: {}
          });
        } else {
          // Transfert hors-ligue → enchère d'officialisation (1h)
          await base44.entities.Auction.create({
            player_id: selectedPlayer.id,
            player_name: selectedPlayer.name,
            player_position: selectedPlayer.position || '',
            ...
            current_bidder_club: buyerClub.name,
            last_bid_at: now, ends_at: endsAt,
            is_external_player: true, transfer_type: 'hors_ligue', status: 'active',
            is_loan: false, reactions: {}
          });
          // Retirer le joueur de son club hors-ligue
          await base44.entities.Player.update(selectedPlayer.id, {
      ...
      await base44.entities.Transfer.create({
        ...
        performance_bonuses: [
          ...bonuses.filter(b => b.condition && b.amount),
          ...(isLoanMode && loanClauses.evolutions !== '' ? [{ condition: `${loanClauses.evolutions} évolution(s) obligatoire(s)`, amount: 0, clause_type: 'evolutions', clause_value: parseInt(loanClauses.evolutions) }] : []),
          ...(isLoanMode && loanClauses.journees !== '' ? [{ condition: `${loanClauses.journees} journée(s) jouée(s) minimum`, amount: 0, clause_type: 'journees', clause_value: parseInt(loanClauses.journees) }] : []),
          ...(isLoanMode && loanClauses.buts !== '' ? [{ condition: `${loanClauses.buts} but(s) minimum`, amount: 0, clause_type: 'buts', clause_value: parseInt(loanClauses.buts) }] : []),
          ...(isLoanMode && loanClauses.pd !== '' ? [{ condition: `${loanClauses.pd} passe(s) décisive(s) minimum`, amount: 0, clause_type: 'pd', clause_value: parseInt(loanClauses.pd) }] : []),
          ...(isLoanMode && loanClauses.penalite !== '' ? [{ condition: `Pénalité si clauses non respectées`, amount: parseInt(loanClauses.penalite), clause_type: 'penalite', clause_value: parseInt(loanClauses.penalite) }] : []),
        ],
  ...
  const handleClose = () => {
    ...
    setLoanRecallFee('');
    setBuybackClause('');
    setLoanClauses({ evolutions: '', journees: '', buts: '', pd: '', penalite: '' });
    setBonuses([]);
      ...
      <DialogContent className="bg-slate-900 border-slate-700 text-white max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        {!selectedPlayer ? (
                ...
                {otherPlayers.slice(0, 40).map(p => (
                  ...
                  >
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      {p.is_on_transfer_list && <span className="text-amber-400 text-xs block">En vente</span>}
                      {sansLigueClubNames.has(p.club_name) && (
                        <span className="text-blue-400 text-xs block">Hors-ligue ✓</span>
                      )}
                      {!sansLigueClubNames.has(p.club_name) && clubsWithoutManager.has(p.club_id) && p.release_clause && (
          ...
          <div className="space-y-4">
            ...
            {/* Infos agent libre (sans club) */}
            {!selectedPlayer.club_id && (
              <div className="p-3 bg-blue-500/10 border border-blue-500/30 rounded-xl">
                <p className="text-blue-300 text-sm font-semibold">👤 Agent libre</p>
                <p className="text-slate-400 text-xs mt-0.5">La signature d'un agent libre crée directement une enchère d'officialisation (1h) sur la Communauté.</p>
              </div>
            )}
            ...
            {isHorsLiguePlayer && (
              <div className="p-3 bg-blue-500/10 border border-blue-500/30 rounded-xl">
                <p className="text-blue-300 text-sm font-semibold">🌍 Club Hors-Ligue</p>
                <p className="text-slate-400 text-xs mt-0.5">L'offre est automatiquement acceptée et mise en enchère d'officialisation (1h) sur la Communauté.</p>
              </div>
            )}

            {/* Clause de libération optionnelle (club avec manager) */}
            {hasClauseOption && (
              <div className="p-3 bg-purple-500/10 border border-purple-500/30 rounded-xl">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-slate-400 text-xs mt-0.5">
                      Montant fixé : <span className="text-purple-300 font-bold">{((selectedPlayer.release_clause) / 1e6).toFixed(2)}M€</span>
                    </p>
                    <p className="text-slate-500 text-xs mt-0.5">
                      ⚠️ Le joueur doit d'abord accepter la clause avant officialisation.
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                {useReleaseClause && (
                  <p className="text-purple-200 text-xs mt-2 bg-purple-500/10 rounded-lg px-3 py-2">
                    📩 Une demande de clause sera envoyée au manager de {selectedPlayer.club_name}. Le transfert ne sera effectif qu'après son acceptation.
                  </p>
                )}
              </div>
            )}
            ...
            {isLoan && (
              <>
                ...
                <div className="space-y-3 p-3 bg-blue-500/10 border border-blue-500/30 rounded-xl">
                  <p className="text-blue-300 text-sm font-semibold">📋 Clauses obligatoires du prêt</p>
                  <p className="text-slate-400 text-xs">Obligations imposées au club emprunteur durant le prêt.</p>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <Label className="text-xs text-slate-300">Évolutions min.</Label>
                      <Input
                        type="number"
                        min="0"
                        value={loanClauses.evolutions}
                        onChange={e => setLoanClauses(c => ({ ...c, evolutions: e.target.value }))}
                        placeholder="Ex: 2"
                        className="bg-slate-800 border-slate-700 text-sm"
                      />
                    </div>
                    <div className="space-y-1">
                      <Label className="text-xs text-slate-300">Journées jouées min.</Label>
                      <Input
                        type="number"
                        min="0"
                        value={loanClauses.journees}
                        onChange={e => setLoanClauses(c => ({ ...c, journees: e.target.value }))}
                        placeholder="Ex: 10"
                        className="bg-slate-800 border-slate-700 text-sm"
                      />
                    </div>
                    <div className="space-y-1">
                      <Label className="text-xs text-slate-300">Buts min.</Label>
                      <Input
                        type="number"
                        min="0"
                        value={loanClauses.buts}
                        onChange={e => setLoanClauses(c => ({ ...c, buts: e.target.value }))}
                        placeholder="Ex: 5"
                        className="bg-slate-800 border-slate-700 text-sm"
                      />
                    </div>
                    <div className="space-y-1">
                      <Label className="text-xs text-slate-300">Passes décisives min.</Label>
                      <Input
                        type="number"
                        min="0"
                        value={loanClauses.pd}
                        onChange={e => setLoanClauses(c => ({ ...c, pd: e.target.value }))}
                        placeholder="Ex: 3"
                        className="bg-slate-800 border-slate-700 text-sm"
                      />
                    </div>
                  </div>
                  <div className="space-y-1 col-span-2 border-t border-blue-500/20 pt-2 mt-1">
                    <Label className="text-xs text-red-300">💸 Pénalité si clauses non respectées (€)</Label>
                    <Input
                      type="number"
                      min="0"
                      value={loanClauses.penalite}
                      onChange={e => setLoanClauses(c => ({ ...c, penalite: e.target.value }))}
                      placeholder="Ex: 5000000"
                      className="bg-slate-800 border-red-500/30 text-sm"
                    />
                    {loanClauses.penalite && (
                      <p className="text-red-300 text-xs">{(parseFloat(loanClauses.penalite) / 1e6).toFixed(2)}M€ reversés automatiquement au club prêteur</p>
                    )}
                  </div>
                </div>
              </>

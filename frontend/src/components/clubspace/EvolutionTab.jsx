...
import { TrendingUp, Search, ArrowUp, Loader2, Info, AlertTriangle, Settings, Trash2, ShoppingCart } from 'lucide-react';
...
export default function EvolutionTab({ club, user }) {
  ...
  const purchasedExtras = activeSeason?.extra_evolutions_used?.[club.id] || 0;
  const MAX_EVOLUTIONS = (activeSeason?.max_evolutions ?? 10) + purchasedExtras;
  ...
  const buyExtraEvolution = async () => {
    ...
    const newVal = (current[club.id] || 0) + 1;
    await base44.entities.Season.update(activeSeason.id, {
      extra_evolutions_used: { ...current, [club.id]: newVal }
    });
    ...
    toast.success('Slot d\'évolution supplémentaire acheté ! (+1 évo, -50M€)');
  };
  ...
  const evolveMutation = useMutation({
    mutationFn: async ({ player, level, club }) => {
      ...
      // Booster aussi les stats détaillées (comme dans le CDF)
      DETAILED_STAT_FIELDS.forEach(f => {
        if (player[f] != null) statsUpdate[f] = Math.min(99, player[f] + level.boost);
      });
      ...
      {activeSeason && (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-amber-300 font-semibold text-sm">Acheter un slot d'évolution supplémentaire</span>
          </div>
          <span className="text-amber-400 font-bold text-sm">50M€</span>
        </button>
      )}

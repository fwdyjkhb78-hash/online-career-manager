...
import { Loader2, Plus, Sparkles, ChevronDown, ChevronUp, X, Trash2, ArrowUpCircle, ArrowUp, TrendingUp, AlertTriangle, ShoppingCart } from 'lucide-react';
...
export default function AcademyTab({ club }) {
  ...
  const EXTRA_EVO_COST = 50_000_000;
  const purchasedExtras = activeSeason?.extra_evolutions_used?.[`academy_${club?.id}`] || 0;
  const MAX_EVOLUTIONS = 10 + purchasedExtras;
  // Compter le nombre de joueurs uniques ayant évolué (pas le nombre total d'évolutions)
  const evolvedPlayerIds = new Set(seasonEvolutions.map(e => e.player_id));
  const totalEvolutionsThisSeason = evolvedPlayerIds.size;
  const limitReached = totalEvolutionsThisSeason >= MAX_EVOLUTIONS;

  const buyExtraEvolution = async () => {
    if ((club?.budget || 0) < EXTRA_EVO_COST) {
      toast.error('Budget insuffisant (50M€ requis)');
      return;
    }
    const current = activeSeason.extra_evolutions_used || {};
    const key = `academy_${club.id}`;
    const newVal = (current[key] || 0) + 1;
    await base44.entities.Season.update(activeSeason.id, {
      extra_evolutions_used: { ...current, [key]: newVal }
    });
    await base44.entities.Club.update(club.id, { budget: (club.budget || 0) - EXTRA_EVO_COST });
    queryClient.invalidateQueries({ queryKey: ['seasons'] });
    queryClient.invalidateQueries({ queryKey: ['all-clubs'] });
    toast.success('Slot d\'évolution CDF supplémentaire acheté ! (+1 évo, -50M€)');
  };
  ...
  const evolveMutation = useMutation({
    mutationFn: async ({ player, level }) => {
      ...
      // Augmenter la valeur : boost * 5M€ par OVR, multiplié par (nbre d'évolutions + 1)
      // Ex: +5 OVR 1ère fois = 5*5M = 25M€ / 2ème fois = 5*5M*2 = 50M€
      const evoCount = getAcademyEvoCount(player.id); // nombre d'évolutions déjà faites
      const valueIncrease = boost * 5_000_000 * (evoCount + 1);
      ...
      const statsUpdate = { overall: newOverall, value: newValue };
...
⚠️ RÈGLE ABSOLUE PAR JOUEUR — À APPLIQUER INDIVIDUELLEMENT POUR CHAQUE JOUEUR GÉNÉRÉ :
→ AVANT de générer les stats d'un joueur, vérifie sa position :
  • Si sa position est "GK" : reflexe_g ENTRE 76 ET 88, plongeon ENTRE 74 ET 85, jeu_a_la_main ENTRE 60 ET 72, degagement ENTRE 62 ET 74, positionnement ENTRE 78 ET 88. Toutes ses stats offensives (finition, tirs_loin, puissance_tir, centres) ENTRE 12 ET 28.
  • Si sa position est CB, LB, RB, CDM, CM, CAM, LW, RW ou ST : reflexe_g ENTRE 12 ET 28, plongeon ENTRE 12 ET 28, jeu_a_la_main ENTRE 10 ET 25, degagement ENTRE 15 ET 30. Ces valeurs NE DOIVENT JAMAIS dépasser 35 pour un non-GK.
⚠️ IL EST INTERDIT de mettre des stats GK élevées (>35) sur un joueur non-GK, même si d'autres joueurs dans la liste sont GK.

- penaltys : OBLIGATOIRE entre 50 et 78 pour TOUS les joueurs non-GK. Pour les GK, entre 40 et 60.
- positionnement pour NON-GK : OBLIGATOIRE réaliste selon le poste. ST/LW/RW : 38-58. CAM : 55-68. CM/CDM : 58-72. CB/LB/RB : 60-75. JAMAIS inférieur à 35, JAMAIS 0.
- ⚠️ AUCUN STAT NE PEUT ÊTRE 0 OU INFÉRIEUR À 10.
...
Réponds en JSON.`,
      ...
      for (const p of result.players) {
        if (p.position === 'GK') {
          // GK : forcer les stats gardien à des valeurs ÉLEVÉES si le LLM les a mal générées
          if (!p.reflexe_g || p.reflexe_g < 60) p.reflexe_g = Math.floor(Math.random() * 13) + 76; // 76-88
          if (!p.plongeon || p.plongeon < 60) p.plongeon = Math.floor(Math.random() * 12) + 74;    // 74-85
          if (!p.positionnement || p.positionnement < 60) p.positionnement = Math.floor(Math.random() * 11) + 78; // 78-88
          if (!p.jeu_a_la_main || p.jeu_a_la_main < 45) p.jeu_a_la_main = Math.floor(Math.random() * 13) + 60; // 60-72
          if (!p.degagement || p.degagement < 45) p.degagement = Math.floor(Math.random() * 13) + 62;           // 62-74
        } else {
          // Non-GK : forcer les stats gardien à des valeurs BASSES
          p.plongeon = Math.min(p.plongeon ?? 20, 30);
          p.jeu_a_la_main = Math.min(p.jeu_a_la_main ?? 18, 28);
          p.degagement = Math.min(p.degagement ?? 20, 30);
          p.reflexe_g = Math.min(p.reflexe_g ?? 18, 28);
        }
  ...
  const STAT_LABELS = {
    ...
    passes_courtes: "Passes courtes", volees: "Volées",
              ...
              <AnimatePresence>
                {expandedId === player.id && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                    ...
                    {/* Stats GK — uniquement pour les gardiens */}
                    {player.position === 'GK' && (
                      <div className="mb-3">
                        <p className="text-purple-300 text-xs font-semibold mb-2 uppercase tracking-wide">Stats Gardien</p>
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                          {[
                            ['reflexe_g', 'Réflexe (G)'],
                            ['plongeon', 'Plongeon (G)'],
                            ['positionnement', 'Positionnement (G)'],
                            ['jeu_a_la_main', 'Jeu à la main (G)'],
                            ['degagement', 'Dégagement (G)'],
                          ].map(([key, label]) => (
                            <div key={key} className="flex items-center justify-between bg-purple-500/10 border border-purple-500/20 rounded-lg px-3 py-1.5">
                              <span className="text-purple-300 text-xs">{label}</span>
                              <span className={`font-bold text-sm ${(player[key] ?? 0) >= 70 ? 'text-emerald-400' : (player[key] ?? 0) >= 60 ? 'text-amber-400' : 'text-red-400'}`}>
                                {player[key] ?? '—'}
                              </span>
                            </div>
                          ))}
                    ...
                    {/* Stats générales — on exclut les stats GK pour tout le monde */}
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                      {Object.entries(STAT_LABELS).map(([key, label]) => {
                        const isGkStat = ['plongeon','jeu_a_la_main','degagement','reflexe_g'].includes(key);
                        if (isGkStat) return null; // géré dans la section dédiée ci-dessus
                        // Pour les GK, le positionnement est déjà affiché dans la section gardien
                        if (key === 'positionnement' && player.position === 'GK') return null;
        ...
        <DialogContent className="bg-slate-900 border-slate-700 text-white max-w-sm">
          ...
          {evoPlayer && (
            <div className="space-y-4">
              ...
              {activeSeason && (
                <button
                  onClick={buyExtraEvolution}
                  disabled={(club?.budget || 0) < EXTRA_EVO_COST}
                  className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl border transition-all
                    ${(club?.budget || 0) >= EXTRA_EVO_COST
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-amber-300 font-semibold text-xs">Acheter un slot CDF supplémentaire</span>
                  </div>
                  <span className="text-amber-400 font-bold text-xs">50M€</span>
                </button>
              )}
              {limitReached && (

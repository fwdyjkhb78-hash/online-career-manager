        ...
        <div className="text-slate-300 text-sm mt-1">
          ...
          {editing && canEditClub ? (
            ...
            <>
              ...
              {a.current_bidder_name && <span className="text-slate-400"> ({a.current_bidder_name})</span>}
...
export default function OfficializationTab({ currentUser, initialSection }) {
  ...
  const createManualMutation = useMutation({
    mutationFn: async (players) => {
      for (const p of players) {
        // Pour les transferts en ligue, transférer le joueur vers le club acheteur
        if (p.transfer_type === 'ligue' && p.player_id) {
          const buyerClub = allClubs.find(c => c.name === p.current_bidder_club);
          if (buyerClub) {
            await base44.entities.Player.update(p.player_id, {
  ...
  // Déduplication : pour un même joueur (player_id ou player_name) vers le même club acheteur,
  // ne garder que l'entrée la plus récente (updated_date ou created_date) pour éviter les doublons
  const deduplicateAuctions = (items) => {
    const seen = new Map();
    for (const a of items) {
      const key = `${a.player_id || a.player_name}__${a.current_bidder_club || ''}`;
      if (!seen.has(key)) {
        seen.set(key, a);
      }
      // On garde le premier (liste déjà triée par -updated_date, donc le plus récent en premier)
    }
  ...
  const agentLibreItems = deduplicateAuctions(completedAuctions.filter(a => isAgentLibre(a)));
  const ligueItems = deduplicateAuctions(completedAuctions.filter(a => !isHorsLigue(a) && !isAgentLibre(a)));
  const horsLigueItems = deduplicateAuctions(completedAuctions.filter(a => isHorsLigue(a) && !isAgentLibre(a)));
  const [agentLibreFilter, setAgentLibreFilter] = useState('all'); // 'all' | 'liberations' | 'signatures'

  const filterItems = (items) => {
    let filtered = items;
    if (activeSection === 'agent_libre') {
      if (agentLibreFilter === 'liberations') filtered = filtered.filter(a => a.current_bidder_club === 'Agent libre' || !a.current_bidder_club);
      if (agentLibreFilter === 'signatures') filtered = filtered.filter(a => a.seller_club_name === 'Agent libre' || !a.seller_club_id);
    }

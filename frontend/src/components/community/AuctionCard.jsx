...
export default function AuctionCard({ auction, currentUser }) {
  ...
  const bidMutation = useMutation({
    mutationFn: async () => {
      const amount = parseFloat(bidAmount) * 1000000; // saisie en M€ → conversion en €
      ...
      await base44.entities.Auction.update(auction.id, {
        ...
        current_bidder_name: currentUser.full_name || currentUser.email || 'Inconnu',
  ...
  const autoProcessMutation = useMutation({
    mutationFn: async () => {
      // Verrou anti-double traitement : tenter de passer status à 'closed' atomiquement
      // Si l'enchère est déjà 'closed' ou 'completed', quelqu'un d'autre l'a déjà traitée → on arrête
      const freshAuction = await base44.entities.Auction.filter({ id: auction.id });
      const current = freshAuction?.[0];
      if (!current || current.status !== 'active') return; // déjà traité
      await base44.entities.Auction.update(auction.id, { status: 'closed' });

      const price = current.current_price || current.starting_price || 0;

      const isHL = current.transfer_type === 'hors_ligue' || current.is_external_player;

      if (isHL) {
        if (current.current_bidder_id) {
          const allClubs = await base44.entities.Club.list();
          const buyerClub = allClubs.find(c => c.name === current.current_bidder_club);
          const buyerClubId = buyerClub?.id;
          if (buyerClubId) {
            if (current.player_id) {
              await base44.entities.Player.update(current.player_id, {
                club_id: buyerClubId, club_name: buyerClub.name,
                is_on_transfer_list: false, asking_price: null
              });
            }
            await base44.entities.Club.update(buyerClubId, { budget: buyerClub.budget - price });
            if (current.seller_club_id) {
              const sellerClub = allClubs.find(c => c.id === current.seller_club_id);
              if (sellerClub) await base44.entities.Club.update(current.seller_club_id, { budget: sellerClub.budget + price });
            }
            await base44.entities.Notification.create({
              user_id: current.current_bidder_id, club_id: buyerClubId, type: 'auction_ended',
              title: `Enchère remportée : ${current.player_name}`,
              message: `Vous avez remporté l'enchère hors ligue pour ${current.player_name} avec ${(price / 1e6).toFixed(2)}M€.`,
              is_read: false, link_page: 'ClubSpace'
            });
          }
        }
        await base44.entities.Auction.create({
          player_name: current.player_name, player_position: current.player_position,
          player_overall: current.player_overall, player_age: current.player_age,
          player_nationality: current.player_nationality, player_image_url: current.player_image_url,
          seller_club_id: current.seller_club_id, seller_club_name: current.seller_club_name,
          starting_price: price, current_price: price,
          current_bidder_id: current.current_bidder_id, current_bidder_name: current.current_bidder_name,
          current_bidder_club: current.current_bidder_club,
          is_external_player: true, transfer_type: 'hors_ligue', status: 'completed',
          ends_at: current.ends_at, reactions: {}
        });
        await base44.entities.Auction.delete(auction.id);
      } else {
        // Ligue (avec ou sans club vendeur — ex: agent libre)
        const allClubs = await base44.entities.Club.list();
        const buyerClub = current.current_bidder_id
          ? allClubs.find(c => c.name === current.current_bidder_club)
          : null;
        const buyerClubId = buyerClub?.id;

        if (current.current_bidder_id && buyerClubId) {
          ...
          // Débiter l'acheteur
          await base44.entities.Club.update(buyerClubId, { budget: (buyerClub.budget || 0) - price });
          // Créditer le vendeur si applicable
          if (current.seller_club_id) {
            const sellerClub = allClubs.find(c => c.id === current.seller_club_id);
            if (sellerClub) await base44.entities.Club.update(current.seller_club_id, { budget: (sellerClub.budget || 0) + price });
          }
          // Notification acheteur
          await base44.entities.Notification.create({
            user_id: current.current_bidder_id, club_id: buyerClubId, type: 'auction_ended',
            title: `Enchère remportée : ${current.player_name}`,
            message: `Vous avez remporté l'enchère pour ${current.player_name} avec ${(price / 1e6).toFixed(2)}M€.`,
            is_read: false, link_page: 'ClubSpace'
          });
          // Notification vendeur
          if (current.seller_club_id) {
            const allUsers = await base44.entities.User.list();
            const sellerUser = allUsers.find(u => u.club_id === current.seller_club_id && u.has_selected_club);
            if (sellerUser) {
              await base44.entities.Notification.create({
                user_id: sellerUser.id, club_id: current.seller_club_id, type: 'auction_ended',
                title: `Joueur vendu : ${current.player_name}`,
                message: `${current.player_name} a été vendu à ${buyerClub.name} pour ${(price / 1e6).toFixed(2)}M€. Budget crédité.`,
                is_read: false, link_page: 'ClubSpace'
              });
            }
          }
        ...
        // Toujours créer l'officialisation (même agent libre sans acheteur)
        await base44.entities.Auction.create({
          player_id: current.player_id || null,
          player_name: current.player_name,
          player_position: current.player_position, player_overall: current.player_overall,
          player_age: current.player_age, player_nationality: current.player_nationality,
          player_image_url: current.player_image_url,
          seller_club_id: current.seller_club_id || null,
          seller_club_name: current.seller_club_name || 'Agent libre',
          starting_price: price, current_price: price,
          current_bidder_id: current.current_bidder_id || null,
          current_bidder_name: current.current_bidder_name || null,
          current_bidder_club: current.current_bidder_club || null,
        ...
        <div>
          ...
          {auction.current_bidder_name ? (
            <div className="flex items-center gap-1.5 mt-1">
              ...
              {auction.current_bidder_club && (
                <span className="text-slate-500 text-xs">· {auction.current_bidder_club}</span>
              )}
            </div>
          ) : (
            <p className="text-slate-500 text-xs mt-0.5 italic">Aucune offre pour le moment</p>
          )}
        </div>
        ...
        <div>
          {showBid ? (
            <div className="flex gap-2">
              ...
              />
              <Button onClick={() => bidMutation.mutate()} disabled={!bidAmount || parseFloat(bidAmount) * 1000000 < minBid || bidMutation.isPending}

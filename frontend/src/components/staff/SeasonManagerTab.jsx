...
export default function SeasonManagerTab() {
  ...
  const handleNewSeason = async () => {
    ...
    try {
      ...
      // 4b. Retour automatique des prêts expirés (sans option d'achat obligatoire)
      try {
        const allTransfers = await base44.entities.Transfer.filter({ offer_type: 'loan', status: 'completed' });
        for (const loan of allTransfers) {
          // Prêts sans option d'achat obligatoire uniquement
          if (loan.loan_mandatory_buy_option && loan.loan_mandatory_buy_option > 0) continue;
          const loanSeasons = loan.loan_seasons || 1;
          // La saison active avant update est activeSeason.season_number
          // On compte combien de saisons se sont écoulées depuis le prêt
          // On utilise created_date pour estimer: si le prêt a été créé à la saison X, il expire à X+loanSeasons
          // On se base sur le numéro de saison actuelle
          const seasonAtLoan = loan.season_at_creation || activeSeason?.season_number || 1;
          const expirySeasonNumber = seasonAtLoan + loanSeasons;
          if (nextSeasonNumber > expirySeasonNumber) {
            // Le prêt est expiré → retour au club d'origine
            const player = await base44.entities.Player.filter({ id: loan.player_id });
            if (player.length > 0 && player[0].club_id === loan.to_club_id) {
              await base44.entities.Player.update(loan.player_id, {
                club_id: loan.from_club_id,
                club_name: loan.from_club_name,
                is_on_transfer_list: false,
              });
              await base44.entities.Transfer.update(loan.id, { status: 'rejected' }); // marquer expiré
            }
          }
        }
      } catch (e) { /* non bloquant */ }
          ...
          <div className="space-y-4">
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4">
              ...
              <ul className="text-slate-300 text-sm mt-2 space-y-1 list-disc list-inside">
                ...
                <li>Les prêts expirés (sans option obligatoire) sont automatiquement renvoyés</li>
                <li>Relégation / montée automatique entre les ligues</li>

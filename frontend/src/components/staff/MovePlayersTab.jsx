...
import { Loader2, ArrowRight, Search, Trash2 } from 'lucide-react';
...
export default function MovePlayersTab() {
  ...
  const deduped = Object.values(
    players.reduce((acc, p) => {
      ...
      // Priorité : a un club > meilleur overall
            ...
            >
              ...
              {selectedPlayer?.id === player.id && (
                <div className="flex flex-col gap-2 items-end" onClick={e => e.stopPropagation()}>
                   {/* Boutons de suppression et mode */}
                   <div className="flex gap-2">
                     ...
                     >
                       {deletePlayerMutation.isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : <Trash2 className="w-3 h-3" />}
                     </button>

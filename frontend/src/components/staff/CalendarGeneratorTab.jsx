...
import { Loader2, CalendarDays, Check, Plus, Trash2, AlertCircle, Wand2, Shuffle, Trophy, AlertTriangle } from 'lucide-react';
import { toast } from 'sonner';
import ChampionshipFormatTab from './ChampionshipFormatTab';
// ChampionshipFormatTab already uses LeagueFormatManager internally
...
export default function CalendarGeneratorTab() {
  ...
  const { data: championships = [] } = useQuery({
    ...
    onSuccess: (data) => {
      if (!championship && data.length > 0) setChampionship(data[0].slug);
    },
  });

  // Init championship au premier chargement
  useEffect(() => {
    if (!championship && championships.length > 0) setChampionship(championships[0].slug);
  }, [championships]);

  const championshipClubs = clubs.filter(c => {
    const arr = c.championships && c.championships.length > 0 ? c.championships : (c.championship ? [c.championship] : []);
    return arr.includes(championship);
  });
  ...
  const [deleteTournamentName, setDeleteTournamentName] = useState('');

  const deleteAllMutation = useMutation({
    mutationFn: async () => {
      const query = deleteTournamentName.trim()
        ? { tournament_name: deleteTournamentName.trim() }
        : {};
      const allMatches = await base44.entities.Match.filter(query, '-created_date', 500);
    ...
    onSuccess: (count) => {
      toast.success(`${count} match${count > 1 ? 's' : ''} supprimé${count > 1 ? 's' : ''} !`);
      queryClient.invalidateQueries({ queryKey: ['matches'] });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-red-500/5 border border-red-500/30 rounded-xl p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-red-400 font-semibold text-sm">Zone dangereuse — Suppression de matchs</h3>
        </div>
        {!confirmDelete ? (
          <Button
            variant="outline"
          ...
          >
            <Trash2 className="w-4 h-4 mr-2" /> Supprimer des matchs
          </Button>
        ) : (
          <div className="space-y-3">
            <div className="space-y-1">
              <Label className="text-slate-300 text-xs">Filtrer par tournoi/championnat (laisser vide = TOUS les matchs)</Label>
              <input
                type="text"
                value={deleteTournamentName}
                onChange={e => setDeleteTournamentName(e.target.value)}
                placeholder="Ex: LDC, Ligue 1, ... ou vide pour tout supprimer"
            ...
            <div className="flex gap-2">
              ...
              >
                {deleteAllMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Trash2 className="w-4 h-4 mr-2" />}
                {deleteTournamentName.trim() ? `Supprimer les matchs "${deleteTournamentName}"` : 'Supprimer TOUS les matchs'}
              </Button>
              <Button size="sm" variant="outline" className="border-slate-600 text-slate-400" onClick={() => { setConfirmDelete(false); setDeleteTournamentName(''); }}>
                Annuler
              </Button>
            </div>
          </div>
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${mode === 'format' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/40' : 'text-slate-400 hover:text-white border border-slate-700'}`}
        >
          <Trophy className="w-4 h-4" /> Format de compétition
        </button>
      </div>
          ...
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5 space-y-4">
            ...
            <div className="flex gap-3 flex-wrap">
              {championships.map(c => (
                <button
                  key={c.slug}
                  onClick={() => { setChampionship(c.slug); setPreview(null); }}
                  className={`px-4 py-2 rounded-lg border text-sm font-medium transition-colors ${championship === c.slug ? 'bg-emerald-500/20 border-emerald-500/60 text-emerald-400' : 'border-slate-600 text-slate-400 hover:border-slate-500'}`}
                >
                  {c.name}
                </button>
              ))}
      ...
      {mode === 'format' && <ChampionshipFormatTab />}

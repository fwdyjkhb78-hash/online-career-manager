...
import TournamentKoGenerator from './TournamentKoGenerator';
...
const TYPE_LABELS = {
  ...
  ldc: { label: 'Phase de ligue LDC', icon: '🏆', color: 'text-amber-400' },
  chapeaux_ldc: { label: 'Chapeaux LDC', icon: '🎩', color: 'text-yellow-400' },
};
      ...
      {showForm && (
        <Card className="bg-slate-800 border-slate-700">
          ...
          <CardContent className="space-y-4">
              ...
              <div>
                ...
                <Select value={form.tournament_type} onValueChange={v => setForm(p => ({ ...p, tournament_type: v }))}>
                  ...
                  <SelectContent className="bg-slate-800 border-slate-700 text-white">
                   ...
                   <SelectItem value="ldc">🏆 Phase de ligue LDC</SelectItem>
                   <SelectItem value="chapeaux_ldc">🎩 Chapeaux LDC</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div>
            ...
            {/* Lien championnat */}
            <div>
              <label className="text-slate-400 text-xs mb-1 block">📌 Lier à un championnat (optionnel — pour afficher les poules dans l'onglet Classement)</label>
              <Select value={form.championship_slug || '_none'} onValueChange={v => setForm(p => ({ ...p, championship_slug: v === '_none' ? '' : v }))}>
                <SelectTrigger className="bg-slate-700 border-slate-600 text-white"><SelectValue placeholder="Aucun" /></SelectTrigger>
                <SelectContent className="bg-slate-800 border-slate-700 text-white">
                  <SelectItem value="_none">Aucun</SelectItem>
                  {championships.map(c => <SelectItem key={c.slug} value={c.slug}>{c.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
              ...
              <Card key={t.id} className="bg-slate-900 border-slate-800">
                ...
                {isExpanded && (
                  <CardContent className="space-y-5 pt-0">
                    ...
                    {/* Phase de ligue LDC / Chapeaux LDC — classement + génération KO */}
                    {(t.tournament_type === 'ldc' || t.tournament_type === 'chapeaux_ldc') && (
                      <div className="border border-amber-500/30 rounded-xl p-4 bg-amber-500/5 space-y-3">
                        <h4 className="text-amber-300 font-semibold text-sm flex items-center gap-2">
                          <Trophy className="w-4 h-4" /> Phase de ligue — Classement & Phase KO
                        </h4>
                        <TournamentKoGenerator tournament={t} />
                      </div>
                    )}
                    {/* Bouton manuel pour déclencher le KO sur n'importe quel tournoi sans génération auto */}
                    {!['ldc','chapeaux_ldc','poules','poules_tableau'].includes(t.tournament_type) && !t.knockout_generated && t.status !== 'upcoming' && (
                      <details className="group">
                        <summary className="cursor-pointer text-slate-400 text-xs flex items-center gap-1.5 select-none py-1">
                          <span className="group-open:rotate-90 transition-transform inline-block">▶</span>
                          Génération de phase KO avancée
                        </summary>
                        <div className="mt-3 border border-amber-500/30 rounded-xl p-4 bg-amber-500/5">
                          <TournamentKoGenerator tournament={t} />
                        </div>
                      </details>
                    )}
                    ...
                    {/* Lien championnat */}
                    <div>
                      <label className="text-slate-400 text-xs mb-1 block">📌 Lié au championnat</label>
                      <Select
                        value={ed.championship_slug !== undefined ? (ed.championship_slug || '_none') : (t.championship_slug || '_none')}
                        onValueChange={v => {
                          const val = v === '_none' ? '' : v;
                          setEditField(t.id, 'championship_slug', val);
                          updateMutation.mutate({ id: t.id, data: { championship_slug: val } });
                        }}
                      >

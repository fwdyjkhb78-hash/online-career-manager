...
export default function MyClub() {
    ...
    mutationFn: async ({ transfer, action, counterAmount }) => {
      ...
      if (action === 'accept') {
        ...
        const buyer = allUsers.find(u => u.club_id === transfer.to_club_id && u.has_selected_club)
          || allUsers.find(u => u.club_id === transfer.to_club_id);

        if (isReleaseClause) {
          // ── CLAUSE DE LIBÉRATION : marquer accepted → négociation joueur ──
          await base44.entities.Transfer.update(transfer.id, {
            status: 'accepted',
            negotiation_history: [...history, { from_club: transfer.from_club_name, amount: transfer.amount, action: 'clause_validated' }]
          });
          ...
          if (buyer) {
            await base44.entities.Notification.create({
              ...
              type: 'transfer_offer',
              title: `Clause acceptée ! Négociez avec ${transfer.player_name}`,
              message: `La clause de libération est validée. Vous devez maintenant négocier avec le joueur pour finaliser.`,
              is_read: false,
              link_page: 'MyClub'
            });
          }
          ...
          if (seller) {
            await base44.entities.Notification.create({
              ...
              message: `Vous avez validé la clause. L'acheteur va maintenant négocier avec le joueur.`,
              is_read: false,
              link_page: 'ClubSpace'
            });
          }
          ...
          // ── OFFRE NORMALE : marquer comme acceptée → l'acheteur doit négocier avec le joueur ──
          await base44.entities.Transfer.update(transfer.id, {
            status: 'accepted',
          ...
          if (buyer) {
            await base44.entities.Notification.create({
              ...
              club_id: transfer.to_club_id,
              type: 'transfer_offer',
              title: `Accord club trouvé ! Négociez avec ${transfer.player_name}`,
              message: `${transfer.from_club_name} accepte ${(transfer.amount / 1e6).toFixed(2)}M€. Vous devez maintenant négocier avec le joueur dans votre onglet Transferts.`,
              is_read: false,
              link_page: 'MyClub'
            });
            base44.functions.invoke('sendNotificationEmail', {
              to_user_id: buyer.id,
              subject: `Accord club conclu pour ${transfer.player_name}`,
              event_type: 'transfer_accepted',
              data: { player_name: transfer.player_name, from_club: transfer.from_club_name, amount: `${(transfer.amount / 1e6).toFixed(2)}M€` }
            }).catch(() => {});
          }
          const seller = allUsers.find(u => u.club_id === transfer.from_club_id && u.has_selected_club)
            || allUsers.find(u => u.club_id === transfer.from_club_id);
          if (seller) {
            await base44.entities.Notification.create({
              user_id: seller.id,
              club_id: transfer.from_club_id,
              ...
              message: `Accord avec ${transfer.to_club_name} à ${(transfer.amount / 1e6).toFixed(2)}M€. L'acheteur va maintenant négocier avec le joueur.`,
              is_read: false,
              link_page: 'ClubSpace'
            });
          }
        ...
        const buyerReject = allUsersReject.find(u => u.club_id === transfer.to_club_id && u.has_selected_club)
          || allUsersReject.find(u => u.club_id === transfer.to_club_id);
        if (buyerReject) {
          await base44.entities.Notification.create({
          ...
          base44.functions.invoke('sendNotificationEmail', {
            to_user_id: buyerReject.id,
            subject: `Offre refusée pour ${transfer.player_name}`,
        ...
        const otherUser = allUsers.find(u => u.club_id === otherClubId && u.has_selected_club)
          || allUsers.find(u => u.club_id === otherClubId);
        if (otherUser) {
          await base44.entities.Notification.create({
          ...
          base44.functions.invoke('sendNotificationEmail', {
            ...
            subject: `Contre-offre pour ${transfer.player_name}`,
            event_type: 'counter_offer',
            data: { player_name: transfer.player_name, from_club: counteringClubName, amount: `${(counterAmount / 1e6).toFixed(2)}M€` }
          }).catch(() => {});
        }
      }
    },
  ...
  const finalizeTransfer = useMutation({
    mutationFn: async ({ transfer, playerRole }) => {
      const now = new Date().toISOString();
      const isClause = transfer.is_release_clause === true;

      const playerData = transfer.player_id
        ? (await base44.entities.Player.filter({ id: transfer.player_id }))?.[0] || null
        : null;
      ...
      if (isClause) {
        // ── CLAUSE : transfert direct (pas d'enchère) ──
        ...
        if (buyerClub) {
          await base44.entities.Club.update(transfer.to_club_id, { budget: (buyerClub.budget || 0) - transfer.amount });
        }
        if (sellerClub) {
          await base44.entities.Club.update(transfer.from_club_id, { budget: (sellerClub.budget || 0) + transfer.amount });
        }

        await base44.entities.Transfer.update(transfer.id, {
          status: 'completed',
          negotiation_history: [...(transfer.negotiation_history || []), { action: 'player_accepted_clause', amount: transfer.amount }]
        });

        if (buyer) {
          await base44.entities.Notification.create({
            user_id: buyer.id, club_id: transfer.to_club_id, type: 'transfer_offer',
            title: `${transfer.player_name} est officiellement à vous !`,
            message: `La clause de libération est finalisée. ${transfer.player_name} rejoint ${transfer.to_club_name} pour ${(transfer.amount / 1e6).toFixed(2)}M€.`,
        ...
        if (seller) {
          await base44.entities.Notification.create({
            ...
            title: `Clause finalisée — ${transfer.player_name} part`,
            message: `${transfer.player_name} a rejoint ${transfer.to_club_name}. ${(transfer.amount / 1e6).toFixed(2)}M€ crédités.`,
            is_read: false, link_page: 'ClubSpace'
          });
        }

      } else {
        // ── OFFRE NORMALE : créer enchère d'officialisation ──
        ...
        await base44.entities.Transfer.update(transfer.id, {
          status: 'completed',
          negotiation_history: [...(transfer.negotiation_history || []), { action: 'player_accepted', amount: transfer.amount }]
        });

        if (seller) {
          await base44.entities.Notification.create({
            user_id: seller.id, club_id: transfer.from_club_id, type: 'transfer_offer',
            title: `${transfer.player_name} accepte de rejoindre ${transfer.to_club_name}`,
            message: `L'enchère d'officialisation est lancée (1h) sur la Communauté.`,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {incomingOffers.map(offer => {
                    // Si la clause est déjà accepted (auto-acceptée pour club avec coach), pas de boutons d'action côté vendeur
                    const isClause = offer.is_release_clause === true;
                     return (
                       <TransferOffer
                         ...
                         transfer={offer}
                         isReceived={!isClause}
                         onAccept={!isClause ? () => handleOffer.mutate({ transfer: offer, action: 'accept' }) : undefined}
                         onReject={!isClause ? () => handleOffer.mutate({ transfer: offer, action: 'reject' }) : undefined}
                         onCounterOffer={!isClause ? (amt) => handleOffer.mutate({ transfer: offer, action: 'counter', counterAmount: amt }) : undefined}
              ...
              <div>
                ...
                <div className="space-y-3">
                  {outgoingOffers.map(offer => {
                   ...
                   const otherSideCountered = !isClause && isNegotiating && offer.last_offer_by === offer.from_club_id;
                   const clubAccepted = offer.status === 'accepted' || isClause; // clause = toujours négociable
                     ...
                     <div key={offer.id}>
                       ...
                       {clubAccepted && (
                         <div className="mt-2 p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-xl flex items-center justify-between gap-3">
                           <div>
                             <p className="text-emerald-300 text-sm font-semibold">✅ Le club a accepté — Négociez avec le joueur</p>
      ...
      <PlayerNegotiationDialog
        ...
        buyerClub={club}
        onNegotiationComplete={({ role }) => {
          if (negotiationDialog.transfer) {
            finalizeTransfer.mutate({ transfer: negotiationDialog.transfer, playerRole: role });
          }

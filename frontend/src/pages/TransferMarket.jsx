...
import {
  ...
  AlertCircle, Zap, Search, X, Users,
  CheckCircle2, Trophy, Globe, RefreshCw, Star, Building2
} from 'lucide-react';
...
import SansLigueTab from '@/components/community/SansLigueTab';
...
export default function TransferMarket() {
  ...
  const [tab, setTab] = useState('transfers'); // 'transfers' | 'officialisations' | 'search' | 'hors-ligue'
  ...
  const [filterOnLoanList, setFilterOnLoanList] = useState(false);
  ...
  const searchResults = useMemo(() => {
    if (!searchQuery && !filterPos && !filterMinOvr && !filterMaxOvr && !filterMinPot && !filterMaxPot && !filterOnSale && !filterHasClause && !filterNoClub && !filterNationality && !filterOnLoanList) return [];
    return allPlayers.filter(p => {
      ...
      if (filterOnLoanList && !p.is_on_loan_list) return false;
  ...
  }, [allPlayers, searchQuery, filterPos, filterMinOvr, filterMaxOvr, filterMinPot, filterMaxPot, filterOnSale, filterHasClause, filterNoClub, filterNationality, filterOnLoanList]);
  ...
  const activateClause = useMutation({
    mutationFn: async (player) => {
      ...
      const sellerClub = player.club_id ? allClubs.find(c => c.id === player.club_id) : null;
      if (player.club_id && !sellerClub) throw new Error('Club vendeur introuvable');
      const buyerClub = effectiveBuyingClub || myClub;
      if (!buyerClub) throw new Error('Club acheteur introuvable');
      if ((buyerClub.budget || 0) < clause) throw new Error('Budget insuffisant');

      // Clause toujours acceptée automatiquement → acheteur négocie avec le joueur
      await base44.entities.Transfer.create({
        player_id: player.id,
        player_name: player.name,
        from_club_id: player.club_id,
        from_club_name: player.club_name,
        to_club_id: buyerClub.id,
        to_club_name: buyerClub.name,
        amount: clause,
        status: 'accepted',
        offer_type: 'transfer',
        is_release_clause: true,
        offer_message: `Clause de libération activée (${(clause / 1e6).toFixed(2)}M€)`,
        last_offer_by: user.id,
      });
      try {
        const users = await base44.entities.User.list();
        const seller = users.find(u => u.club_id === player.club_id && u.has_selected_club);
        if (seller) {
          await base44.entities.Notification.create({
            user_id: seller.id,
            club_id: player.club_id,
            type: 'transfer_offer',
            title: `Clause activée — ${player.name}`,
            message: `${buyerClub.name} a activé la clause de libération de ${player.name} (${(clause / 1e6).toFixed(2)}M€). L'acheteur va négocier avec le joueur.`,
            is_read: false,
            link_page: 'ClubSpace'
          });
        }
        const buyer = users.find(u => u.club_id === buyerClub.id && u.has_selected_club) || users.find(u => u.club_id === buyerClub.id);
        if (buyer) {
          await base44.entities.Notification.create({
            user_id: buyer.id,
            club_id: buyerClub.id,
            type: 'transfer_offer',
            title: `Clause activée ! Négociez avec ${player.name}`,
            message: `La clause est validée. Allez dans Mon Club → Transferts pour négocier avec le joueur.`,
            is_read: false,
            link_page: 'MyClub'
          });
        }
    ...
    onSuccess: () => { setSelectedPlayer(null); queryClient.invalidateQueries({ queryKey: ['all-players-transfer'] }); queryClient.invalidateQueries({ queryKey: ['auctions'] }); }
  });
      ...
      try {
        ...
        if (seller) {
          ...
          base44.functions.invoke('sendNotificationEmail', {
            ...
            subject: `${isLoanOffer ? 'Offre de prêt' : 'Offre'} reçue pour ${selectedPlayer.name}`,
            event_type: 'transfer_offer_received',
            data: { player_name: selectedPlayer.name, from_club: buyerClub.name, amount: `${(parseInt(offerAmount) / 1e6).toFixed(2)}M€`, offer_type: isLoanOffer ? 'Prêt' : 'Transfert' }
          }).catch(() => {});
        }
      } catch (e) {}
    },
  ...
  // Suggestions de nationalités basées sur les joueurs existants
  ...
  const hasActiveSearch = searchQuery || filterPos || filterMinOvr || filterMaxOvr || filterMinPot || filterMaxPot || filterOnSale || filterHasClause || filterNoClub || filterNationality || filterOnLoanList;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="border-b border-slate-800 bg-slate-950/80 backdrop-blur-lg sticky top-16 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {/* Tabs */}
                   <div className="flex bg-slate-800 rounded-xl p-1 gap-1 overflow-x-auto scrollbar-hide">
                     <button onClick={() => setTab('transfers')}
                       className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${tab === 'transfers' ? 'bg-emerald-500 text-white shadow' : 'text-slate-400 hover:text-white'}`}>
                       <ArrowRightLeft className="w-3.5 h-3.5 sm:w-4 sm:h-4" /><span className="hidden sm:inline">Transferts</span><span className="sm:hidden">Transf.</span>
                     </button>
                     <button onClick={() => setTab('search')}
                       className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${tab === 'search' ? 'bg-violet-500 text-white shadow' : 'text-slate-400 hover:text-white'}`}>
                       <Search className="w-3.5 h-3.5 sm:w-4 sm:h-4" /><span className="hidden sm:inline">Recherche</span><span className="sm:hidden">Rech.</span>
                     </button>
                     <button onClick={() => setTab('officialisations')}
                       className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${tab === 'officialisations' ? 'bg-blue-500 text-white shadow' : 'text-slate-400 hover:text-white'}`}>
                       <CheckCircle2 className="w-3.5 h-3.5 sm:w-4 sm:h-4" /><span className="hidden sm:inline">Officialisations</span><span className="sm:hidden">Offic.</span>
                     </button>
                     <button onClick={() => setTab('hors-ligue')}
                       className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${tab === 'hors-ligue' ? 'bg-orange-500 text-white shadow' : 'text-slate-400 hover:text-white'}`}>
                       <Building2 className="w-3.5 h-3.5 sm:w-4 sm:h-4" /><span className="hidden sm:inline">Hors-Ligue</span><span className="sm:hidden">H.Ligue</span>
                     </button>
                   </div>
          </div>
            ...
            <div className="bg-slate-900 border border-violet-500/30 rounded-2xl p-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div onClick={() => setFilterOnLoanList(!filterOnLoanList)}
                   className={`bg-slate-800 border rounded-xl p-2.5 flex items-center gap-2 cursor-pointer transition-all ${filterOnLoanList ? 'border-cyan-500/60 bg-cyan-500/10' : 'border-slate-600 hover:border-slate-500'}`}>
                   <div className={`w-5 h-5 rounded flex items-center justify-center shrink-0 ${filterOnLoanList ? 'bg-cyan-500' : 'bg-slate-600'}`}>
                     <span className="text-xs">🤝</span>
                   </div>
                   <p className={`text-xs font-bold ${filterOnLoanList ? 'text-cyan-300' : 'text-slate-400'}`}>Disponible prêt</p>
                  </div>
                  </div>
              </div>

              {/* Nationalité */}
              <div className="col-span-2 sm:col-span-4 bg-slate-800 border border-slate-600 rounded-xl p-3 flex flex-col gap-2">
                  ...
                  {showNatSuggestions && nationalitySuggestions.length > 0 && (
                    <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-slate-800 border border-slate-600 rounded-lg overflow-hidden shadow-xl">
                      {nationalitySuggestions.map(nat => (
                        <button
                          key={nat}
                          onMouseDown={() => { setNationalityInput(nat); setFilterNationality(nat); setShowNatSuggestions(false); }}
              ...
              <div className="mt-3 flex justify-end">
                <button onClick={() => { setFilterPos(''); setFilterMinOvr(''); setFilterMaxOvr(''); setFilterMinPot(''); setFilterMaxPot(''); setFilterOnSale(false); setFilterHasClause(false); setFilterNoClub(false); setFilterOnLoanList(false); setSearchQuery(''); setFilterNationality(''); setNationalityInput(''); }}
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {player.is_on_loan_list && <Badge className="bg-cyan-500/20 text-cyan-300 border-cyan-500/30 text-xs px-1.5 py-0">🤝 Prêt dispo</Badge>}
        ...
        {/* ── TAB CONTENT ── */}
         {tab === 'transfers' && (
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
             {/* Club sidebar */}
             <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl overflow-hidden h-fit max-h-[60vh] lg:max-h-none lg:sticky lg:top-36">
      ...
      {/* ── HORS-LIGUE TAB ── */}
      {tab === 'hors-ligue' && (
        <SansLigueTab currentUser={user} userClub={myClub} />
      )}
      ...
      <Dialog open={!!selectedPlayer} onOpenChange={() => { setSelectedPlayer(null); setIsLoanOffer(false); setLoanBuyOption(''); setLoanMandatoryBuyOption(''); }}>
        <DialogContent className="bg-slate-900 border-slate-700 text-white max-w-sm sm:max-w-lg w-[95vw] max-h-[85vh] overflow-y-auto">
          <DialogHeader>
          ...
          {selectedPlayer && (
            <div className="space-y-5">
              ...
              {/* ── CAS : Joueur SANS CLUB → offre directe ── */}
              {!selectedPlayer?.club_id && (
                <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl space-y-3">
                  <div className="flex items-center gap-2">
                    <Users className="w-4 h-4 text-blue-400" />
                    <p className="text-blue-300 font-semibold text-sm">Agent libre — Recrutement direct</p>
                  </div>
                  <p className="text-slate-400 text-xs">Le recrutement d'un agent libre est direct — le joueur rejoint immédiatement votre club et l'officialisation est publiée sur la Communauté.</p>
                  <div className="space-y-2">
                    <Label className="text-slate-300 text-xs">Montant de l'offre (€)</Label>
                    <Input type="number" value={offerAmount} onChange={e => setOfferAmount(e.target.value)} placeholder="Ex: 5000000" className="bg-slate-800 border-slate-700 h-8 text-sm" />
                    {offerAmount && <p className="text-slate-400 text-xs">{(parseFloat(offerAmount)/1e6).toFixed(2)}M€</p>}
                  </div>
                  {activeBuyingClub && parseInt(offerAmount) > (activeBuyingClub.budget || 0) && <p className="text-red-400 text-xs">⚠️ Budget insuffisant</p>}
                  ...
                  <Button
                    onClick={async () => {
                      ...
                      const offerAmt = parseInt(offerAmount) || 0;
                      await base44.entities.Player.update(selectedPlayer.id, {
                        club_id: buyerClub.id,
                        club_name: buyerClub.name,
                        is_on_transfer_list: false,
                        asking_price: null,
                      });
                      if (offerAmt > 0) {
                        await base44.entities.Club.update(buyerClub.id, { budget: (buyerClub.budget || 0) - offerAmt });
                      }
                      await base44.entities.Transfer.create({
                        player_id: selectedPlayer.id,
                        player_name: selectedPlayer.name,
                        from_club_id: null,
                        from_club_name: 'Agent libre',
                        to_club_id: buyerClub.id,
                        to_club_name: buyerClub.name,
                        amount: offerAmt,
                        status: 'completed',
                    ...
                    disabled={!user?.has_selected_club || !myClub || activateClause.isPending || (parseInt(offerAmount) > (activeBuyingClub?.budget || 0))}
                    className="w-full bg-blue-500 hover:bg-blue-600 text-white"
                  >
                    <Users className="w-4 h-4 mr-2" />
                    Recruter l'agent libre
                  </Button>
                </div>
              )}

              {/* ── CAS : Club SANS MANAGER → clause directe (enchère immédiate) ── */}
              {selectedPlayer?.club_id && !allClubs.find(c => c.id === selectedPlayer.club_id)?.manager_id && (
                <div className="p-4 bg-purple-500/10 border border-purple-500/30 rounded-xl space-y-3">
                  <div className="flex items-center gap-2">
                    <Zap className="w-4 h-4 text-purple-400" />
                    <p className="text-purple-300 font-semibold text-sm">Club sans coach — Clause de libération</p>
                  </div>
                  {selectedPlayer.release_clause > 0 ? (
                    <>
                      <p className="text-slate-400 text-xs">Ce joueur appartient à un club sans entraîneur. La clause de libération est acceptée automatiquement : <span className="text-purple-300 font-bold">{(selectedPlayer.release_clause / 1e6).toFixed(2)}M€</span>. Vous devrez ensuite négocier avec le joueur dans Mon Club.</p>
                      <Button
                        onClick={() => activateClause.mutate(selectedPlayer)}
                        disabled={!user?.has_selected_club || !myClub || (myClub.budget || 0) < selectedPlayer.release_clause || activateClause.isPending}
                        className="w-full bg-purple-500 hover:bg-purple-600 text-white"
                      >
                        {activateClause.isPending ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Zap className="w-4 h-4 mr-2" />}
                        Activer la clause — négocier avec le joueur
                      </Button>
                      <p className="text-slate-500 text-xs text-center">Clause acceptée automatiquement — négociez ensuite avec le joueur dans Mon Club.</p>
                    </>
                  ) : (
                    <p className="text-slate-400 text-xs">⚠️ Ce joueur n'a pas de clause de libération définie. Contactez le staff pour débloquer ce transfert.</p>
                  )}
                </div>
              )}

              {/* ── CAS NORMAL : joueur avec club ET manager ── */}
              {selectedPlayer?.club_id && allClubs.find(c => c.id === selectedPlayer.club_id)?.manager_id && (
              <div className="space-y-4">
              {/* Clause optionnelle */}
              {selectedPlayer?.release_clause > 0 && (
                <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl space-y-3">
                  ...
                  >
                    ...
                    <p className="text-slate-500 text-xs text-center">La clause est acceptée automatiquement — négociez ensuite avec le joueur dans Mon Club.</p>
                </div>
              )}
